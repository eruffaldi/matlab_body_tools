function [skel, channels, frameLength] = bvhReadFile(fileName,sc)
...
%	Description:
%
%	[SKEL, CHANNELS, FRAMELENGTH] = BVHREADFILE(FILENAME) reads a bvh
%	file into a tree structure and obtains the channels.
%	 Returns:
%	  SKEL - the tree structure of the skeleton.
%	  CHANNELS - the channels of the data read in.
%	  FRAMELENGTH - the time for each frame.
%	 Arguments:
%	  FILENAME - the name of the file to read in, in bvh format.
%	
%
%	See also
%	ACCLAIMREADSKEL, BVHPLAYFILE, BVHWRITEFILE


%	Copyright (c) 2005 Neil D. Lawrence
% 	bvhReadFile.m CVS version 1.3
% 	bvhReadFile.m SVN version 398
% 	last update 2009-06-04T16:53:19.000000Z
   ...
   case 'OFFSET'
    ...
    if length(offsets) == 3
      for i = 1:3
        ...
        skel.tree(jointNo).offset(1, i) = offset*sc;
      end
    else 
...
skel.nchannels = size(channels,2);
skel.frameLength = frameLength;

channels = smoothAngleChannels(channels, skel);
if sc ~= 1
    channels(:,needscale) = channels(:,needscale)*sc;
end
